...
(defn sample-to-png [d sample image-path]
  ...
  (try
    ...
    (catch Exception e
      (timbre/error "Sample to png error: " e)
      {:error (str "Sample to png error occured: " e)
  ...
  (try
    ...
    (if-let [driver (get-free-driver (-> generator :drivers-queue))]
      (let [images-dir (-> generator :conf :images-dir)
            ...
            res (sample-to-png driver sample image-path)]
